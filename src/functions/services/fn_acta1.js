const mappingActa = async (json) => {
  const style = `<style>
* {
  font-family: verdana;
  font-size: 10px;
}
 
table, th, td {
  border: 1px solid darkgray;
  border-collapse: collapse;
  margin-bottom: 40px;
  padding: 5px;
  word-break: break-all;

}
 
.principal {
  display: flex;
  justify-content: center
}
 
.header {
  width: 120px;
  height: 40px;
  text-align: left;
}
 
.p-sp {
  margin-left: 540px;
  margin-bottom: 40px
}
 
h3 {
  font-family: verdana;
  text-align: center;
  margin-bottom: 30px;
}
 
.td-special {
  width: 20px;
}
 
.tabla-firma {
  margin-top: 50px;
  display: flex;
  justify-content: center
}
 
.gris {
  background-color: lightgray;
}
 
.cuadrado {
  width: 350px;
  height: 150px;
  background: white;
  border: 2px solid darkgray;
  margin: 0 auto;
  font-weight: 800;
  padding: 5px;
}
 
.cobertura-seguro{
  margin-top: 80px;
  margin-left: 100px;
  margin-right: 100px;
  font-size: 18px;
}
 
li {
  padding: 10px;
  font-size: 11px;
  text-align: justify;
}
 
.imagenes {
  text-align: center;
  padding: 10px;
}
 
.page-break {
  page-break-before: always; /* Forzar un salto de página antes de este elemento */
}
</style>`;
  const htmlContent = `
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="css.css">
    ${style}
</head>
<body>
  <div class="header">
    <img src="data:image/svg+xml;base64,">
  </div>
  <p class="p-sp"><b> N° pedido asociado:</b> </p>
  <h3>ACTA ENTREGA / RECEPCIÓN DE VEHÍCULOS N°: </h3>
  <div class="principal">
    <table>
      <tr>
        <td><b>Preparado por: </b>${json.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Fecha: </b>${json.DATOS_INSPECCION.fecha_inspeccion}</td>
        <td><b>Hora Recepción: </b> / <b>Hora Inspección: </b>${
          json.DATOS_INSPECCION.hora_recepcion
        }</td>
      </tr>
      <tr>
        <td><b>Patente: </b>${json.DATOS_INSPECCION.patente_vehiculo}</td>
        <td><b>Marca: </b>${json.DATOS_INSPECCION.marca_vehiculo}</td>
        <td><b>Modelo: </b>${json.DATOS_INSPECCION.modelo_vehiculo}</td>
      </tr>
      <tr>
        <td><b>Chasis: </b>${json.DATOS_INSPECCION.chasis_vehiculo}</td>
        <td><b>Color: </b>${json.DATOS_INSPECCION.color_vehiculo}</td>
        <td><b>Año: </b>${json.DATOS_INSPECCION.anio_vehiculo}</td>
      </tr>
      <tr>
        <td colspan=3><b>Cliente: </b>${
          json.DATOS_INSPECCION.preparado_por
        }</td>
      </tr>
      <tr>
        <td><b>Kilometraje: </b>${json.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Combustible: </b>${json.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Tipo de Servicio: </b>${json.DATOS_INSPECCION.preparado_por}</td>
      </tr>
      <tr>
        <td><b>Nombre Chofer: </b>${json.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Celular: </b>${json.DATOS_INSPECCION.preparado_por}</td>
        <td><b>Mail: </b>${json.DATOS_INSPECCION.preparado_por}</td>
      </tr>
      <tr>
        <td colspan=3><b>Observaciones: ${
          json.DATOS_INSPECCION.observaciones_inspeccion
        }</b> </td>
      </tr>
    </table>
  </div>
 
 
  <div class="principal">
    <table style="width: 100%">
      <thead class="gris">
        <tr>
          <th>Nombre del Artículo</th>
          <th>Existe</th>
          <th>Estado</th>
          <th>Comentario</th>
        </tr>
      </thead>
      <tbody>
        ${json.ACCESORIOS_INSPECCION.map(
          (articulo) => `
        <tr>
          <td>${articulo.nombreArticulo}</td>
          <td>${articulo.Existe}</td>
          <td>${articulo.Estado}</td>
          <td>${articulo.Comentario}</td>
        </tr>
        `
        ).join("")}
      </tbody>
    </table>
  </div>
 
 
  <div class="page-break";>
    <div class="header">
      <img src="data:image/svg+xml;base64,">
    </div>
 
    <div class="tabla-firma">
      <table>
        <tr>
          <th>POR GAMALEASING SPA</th>
          <th>USUARIO CLIENTE</th>
        </tr>
        <tr>
          <td><b>NOMBRE:</b> ${json.DATOS_INSPECCION.preparado_por}</td>
          <td><b>NOMBRE:</b> ${json.DATOS_INSPECCION.preparado_por}</td>
        </tr>
        <tr>
          <td><b>RUT:</b> ${json.DATOS_INSPECCION.preparado_por}</td>
          <td><b>RUT:</b> ${json.DATOS_INSPECCION.preparado_por}</td>
        </tr>
      </table>
    </div>
  </div>
 
    <div class="cuadrado">
      FIRMA CLIENTE:
    </div>
 
    <div class="cobertura-seguro">
      <ol>
        <li>
          Cobertura del Seguro se hará efectiva solamente si se realiza la constancia policial inmediatamente ocurrido el Siniestro.
        </li>
        <li>
          Todo vehículo de reemplazo considera depósito de combustible lleno en su entrega, el cual debe retornar de igual manera,
          en caso contrario la diferencia será facturado al cliente.
        </li>
      </ol>
    </div>
 
    <div class="imagenes">
      ${json.FOTOS_INSPECCION.map(
        (imagen) => `
        <div class="page-break";>
          <div class="header">
            <img src="data:image/svg+xml;base64,">
          </div>
          <h5>${imagen.nombre_foto}</h5>
            <img src="${("data:image/png;base64,", imagen.$content)}">
              <p><b>Comentario: </b>${imagen.comentario_foto}</p>
        </div>
      `
      ).join("")}
    </div>
</body>
</html>
  `;
  return htmlContent;
};

const Acta1 = async (json) => {
  const respuesta = await mappingActa(json);
  return respuesta;
};

export default Acta1;
